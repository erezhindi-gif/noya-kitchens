...
export default async function Footer() {
  ...
  const address     = s?.address     || "המלאכה 4, נתניה";
  const whatsapp    = s?.whatsapp    || "972502808180";
  const facebookUrl = s?.facebookUrl || "https://www.facebook.com/profile.php?id=100054656460007";
  const tagline     = s?.footerTagline   || "מטבחים ונגרות בהתאמה אישית.\nמעל 30 שנות ניסיון — ארז הנדי.";
  const copyright   = s?.footerCopyright || "© 2025 NOYA מטבחים ונגרות — ארז הנדי. כל הזכויות שמורות.";
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mb-10">
          ...
          <div>
            ...
            <p className="text-[#6b6b6b] text-sm leading-relaxed whitespace-pre-line">{tagline}</p>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-[#6b6b6b]">
              <li><a href={`tel:${phone.replace(/-/g,'')}`} className="hover:text-[#8B6F47] transition-colors flex items-center gap-2">📞 {phone}</a></li>
              <li><a href={`mailto:${email}`} className="hover:text-[#8B6F47] transition-colors flex items-center gap-2">✉️ {email}</a></li>
